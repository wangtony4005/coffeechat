  ...
  return (
    <>
      ...
      >
        <Link
          to="/shop"
          className="text-2xl font-bold text-mocha-color tracking-wide"
        >
          Mocha Shop
        </Link>
        <nav className="hidden lg:flex space-x-8 text-lg">
          <Link
            to="/profile"
            className="hover:text-mocha-color transition"
            state={{ firstName, lastName, username, email, role }}
          >
            Home
          </Link>
        </nav>
        ...
        {isMenuOpen && (
          <div className="absolute top-16 right-0 w-full bg-nav-color shadow-lg lg:hidden flex flex-col items-center py-4 space-y-4">
            <Link
              to="/find"
              className="text-lg hover:text-mocha-color transition"
